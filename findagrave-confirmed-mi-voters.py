...
def main():
    ...
    def save(people):
        ...
        if not os.path.isfile("confirmed_dead-90.csv"):
            f = open("confirmed_dead-90.csv", "w")
            f.write("First Name, Last Name, Birth Year, Birth_Month, Zip_Code, Matches (here onward)\n")
        else:
            f = open("confirmed_dead-90.csv", "a")

        print(first_name, last_name, birth_year, len(people))
        # print(people)
        matches = ", ".join([p["href"] for p in people[:-1]]) + str(people[-1]["href"])
        # print(matches)
        # re = michigan.get_reg_status(first_name, last_name, birth_month, birth_year, zip_code)
   
        f.write(f"{first_name},{last_name},{birth_year},{birth_month},{zip_code}," + matches + "\n")
    ...
    df = pd.read_csv("90+MIVOTERS.csv")
